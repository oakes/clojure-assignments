...
Save, reload, and run `(def rooms (create-rooms))` again. Then run `(get rooms 0)` and note that we get back a vector of rooms this time. If you want to retrieve a specific room, such as the one at `0,0`, we can go several layers deep using the `get-in` function. For example, `(get-in rooms [0 0])` will get the first value of the outer vector, and then the first value of the inner one.
