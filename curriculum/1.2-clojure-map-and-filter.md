...
Our next task is to turn each line in `people` into a clojure hash map, using the strings in `header` as the keys. We can do this with the `zipmap` function. It simply takes to collections and mixes them together, using the first as the keys and the second as the values. For example, `(zipmap ["first_name" "last_name"] ["Martha" "Jenkins"])` will return `{"first_name" "Martha", "last_name" "Jenkins"}`. We'll need to use `map` so this is done to each line:
