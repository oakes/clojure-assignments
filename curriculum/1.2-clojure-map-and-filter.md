...
Let's make it possible for the user to type which country to filter by. We can read console input with the `read-line` function. It simply returns the string that the turn typed, so we can just include it in the `let` like this:
...
Save, reload, and run `(-main)` in the REPL again, and type the name of a country. Nightcode won't detect that a new file appeared until you change the focus of the window, so switch to another application and then switch back. You should now see a file called `filtered_people.edn` there with the results inside of it. We now have a neat little program, and if you stop the REPL you should be able to hit "Run" and see the same behavior you saw in the REPL. This completes our first experience with the rapid feedback of REPL-driven development!
